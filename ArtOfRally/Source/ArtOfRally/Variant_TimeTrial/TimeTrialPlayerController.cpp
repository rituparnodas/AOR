	...
	if (IsLocalPlayerController())
	{
		if (ShouldUseTouchControls())
		{
			...
			if (MobileControlsWidget)
			{
				...
				UE_LOG(LogArtOfRally, Error, TEXT("Could not spawn mobile controls widget."));
		...
		if (UIWidget)
		{
			...
			UE_LOG(LogArtOfRally, Error, TEXT("Could not spawn Time Trial UI widget."));

		}
		

		// spawn the UI widget and add it to the viewport
		VehicleUI = CreateWidget<UArtOfRallyUI>(this, VehicleUIClass);
		...
		if (VehicleUI)
		{
			...
			UE_LOG(LogArtOfRally, Error, TEXT("Could not spawn vehicle UI widget."));
...
void ATimeTrialPlayerController::OnPossess(APawn* InPawn)
{
	...
	VehiclePawn = CastChecked<AArtOfRallyPawn>(InPawn);
...
void ATimeTrialPlayerController::OnPawnDestroyed(AActor* DestroyedPawn)
{
	...
	if (ActorList.Num() > 0)
	{
		...
		if (AArtOfRallyPawn* RespawnedVehicle = GetWorld()->SpawnActor<AArtOfRallyPawn>(VehiclePawnClass, SpawnTransform))
		{
			// possess the vehicle
